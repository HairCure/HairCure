...
//  HairCure
//
//  Scan Report / Plan Results screen.
//
//  Layout top → bottom:
//   1. Nav bar  "Scan Report"
//   2. Scan photo row  (3 placeholder thumbnails + timestamps)
//   3. Horizontal paging cards (2 cards, page dots):
//      Card A — Hair Analysis Results (density %, stage, scalp)
//      Card B — Lifestyle Scores (Poor/Moderate/Good badge + 4 rows)
//   4. Plan badge card  (circle + stage + lifestyle pills)
//   5. "Recommended Plan" section — icon cards stacked
//   6. Daily Targets strip  (5 cells incl. sleep)
//   7. "Get Started" CTA
//
...
struct PlanResultsView: View {
    let onStart:  () -> Void
    var isSheet:  Bool = false          // true when opened from HomeView "View Details"
    ...
    private var ctaButton: some View {
        VStack(spacing: 0) {
            ...
            Button { isSheet ? dismiss() : onStart() } label: {
                Text(isSheet ? "Continue" : "Get Started")
                    .hcPrimaryButton()
            }
            .padding(.horizontal, 20)
            .padding(.bottom, 36)
