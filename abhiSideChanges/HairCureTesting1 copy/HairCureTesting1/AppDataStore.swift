...
class AppDataStore {
    ...
    private func seedQuestions() {
        ...
        questions.append(q3)
        ["Itching", "Dryness", "Burning / inflammation", "Oily scalp", "None"].enumerated().forEach { i, text in
        ...
        let q6opts = [
            ...
            ("Often junk / fast food",       Float(3.0)),
            ("Very poor / skipping meals",   Float(1.0))
        ]
        ...
        let q8 = Question(id: UUID(), questionType: .singleChoice,
            questionText: "How often do you wash / shampoo your hair?",
    ...
    private func seedSleepAndWater(userId: UUID) {
        ...
        [(1, 1, 6, Float(5.0)), (2, 0, 5, Float(5.5)),
         (3, 2, 7, Float(5.0)), (4, 23, 6, Float(7.0)),
         (5, 1, 6, Float(5.0))].forEach { daysAgo, bedHr, wakeHr, hrs in
            let base = cal.date(byAdding: .day, value: -daysAgo, to: Date())!
            let bed  = cal.date(bySettingHour: bedHr,  minute: 0, second: 0, of: base)!
            let wake = cal.date(bySettingHour: wakeHr, minute: 0, second: 0, of: base)!
            sleepRecords.append(SleepRecord(id: UUID(), userId: userId, date: base,
                bedTime: bed, wakeTime: wake, alarmEnabled: true,
                alarmTime: wake, hoursSlept: hrs))
        }
        waterIntakeLogs.append(WaterIntakeLog(id: UUID(), userId: userId,
            date: Date(), cupSize: "medium",
            cupSizeAmountInML: 250, loggedAt: Date()))
    }
    ...
    func addOrIncrementFood(_ food: Food, to mealEntryId: UUID) {
        if let idx = mealFoods.firstIndex(where: { $0.mealEntryId == mealEntryId && $0.foodId == food.id }) {
            mealFoods[idx].quantity += 1.0
        } else {
            mealFoods.append(MealFood(id: UUID(), mealEntryId: mealEntryId,
                                      foodId: food.id, quantity: 1.0))
        }
        updateMealEntryTotals(mealEntryId: mealEntryId)
    }

    func incrementFood(mealFoodId: UUID, mealEntryId: UUID) {
        if let idx = mealFoods.firstIndex(where: { $0.id == mealFoodId }) {
            mealFoods[idx].quantity += 1.0
        }
        updateMealEntryTotals(mealEntryId: mealEntryId)
    }
    ...
    func decrementOrRemoveFood(mealFoodId: UUID, mealEntryId: UUID) {
        if let idx = mealFoods.firstIndex(where: { $0.id == mealFoodId }) {
            if mealFoods[idx].quantity > 1.0 {
                mealFoods[idx].quantity -= 1.0
            } else {
                mealFoods.remove(at: idx)
            }
        }
        updateMealEntryTotals(mealEntryId: mealEntryId)
    }
    ...
    func mealGoalMessage(for entry: MealEntry) -> String {
        ...
        case .exceeded:
            ...
            return "⚠️ \(over) kcal over target — logged anyway."
        case .under:
    ...
    func mindfulMinutes(for date: Date) -> Int {
        ...
        return mindfulSessions
            .filter { $0.userId == currentUserId &&
                      Calendar.current.startOfDay(for: $0.sessionDate) == startOfDay }
            .reduce(0) { $0 + $1.minutesCompleted }
    }
    
    var dailyMindfulTarget: Int {
        return appPreferences.first(where: { $0.userId == currentUserId })?.dailyMindfulMinutesGoal ?? 0
    }
    ...
    func totalCalorieTarget(for date: Date) -> Float {
        return activeNutritionProfile?.tdee ?? 2000
    }
}
