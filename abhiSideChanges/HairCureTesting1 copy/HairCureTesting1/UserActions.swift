...
//    }
    ...
    func completeSession(contentId: UUID, minutesCompleted: Int) -> ActionResult {
        ...
        if let idx = todaysPlans.firstIndex(where: {
            ...
            todaysPlans[idx].isCompleted = todaysPlans[idx].minutesCompleted >= todaysPlans[idx].minutesTarget
        }
    ...
    var favouriteMindEaseContents: [MindEaseCategoryContent] {
        let ids = userFavorites
            .filter { $0.userId == currentUserId && $0.contentType == "mindEase" }
            .map { $0.contentId }
        return mindEaseCategoryContents.filter { ids.contains($0.id) }
    }
    ...
    /// Total macros consumed today — for home macro detail charts.
    func todaysTotalMacros() -> (protein: Double, carbs: Double, fat: Double) {
        let entries = todaysMealEntries()
        let p = entries.reduce(0) { $0 + Double($1.proteinConsumed) }
        let c = entries.reduce(0) { $0 + Double($1.carbsConsumed) }
        let f = entries.reduce(0) { $0 + Double($1.fatConsumed) }
        return (p, c, f)
    }
