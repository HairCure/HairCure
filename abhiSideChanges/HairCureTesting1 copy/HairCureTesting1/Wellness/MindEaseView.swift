...
struct MindEaseView: View {
    ...
    var body: some View {
        ScrollView(showsIndicators: false) {
            VStack(alignment: .leading, spacing: 24) {
                ...
                // Favourites
                if !store.favouriteMindEaseContents.isEmpty {
                    favouritesSection
                }
    ...
    private var favouritesSection: some View {
        VStack(alignment: .leading, spacing: 14) {
            ...
            ScrollView(.horizontal, showsIndicators: false) {
                HStack(spacing: 14) {
                    ForEach(store.favouriteMindEaseContents) { content in
                        MindEaseFavouriteCard(content: content) {
    ...
    var body: some View {
        HStack(spacing: 14) {
            ...
            Button(action: onStart) {
                let buttonText = plan.isCompleted ? "Completed" : (plan.minutesCompleted > 0 ? "Pending" : "Start")
                let buttonColor = plan.isCompleted ? Color.green : (plan.minutesCompleted > 0 ? Color.orange : purple)
...
struct MindEaseFavouriteCard: View {
    let content: MindEaseCategoryContent
    let onTap: () -> Void
        ...
        Button(action: onTap) {
            VStack(alignment: .leading, spacing: 10) {
                RoundedRectangle(cornerRadius: 12)
                    .fill(Color(red: 0.40, green: 0.30, blue: 0.85).opacity(0.15))
                    .frame(width: 160, height: 100)
                    .overlay(
                        Image(systemName: content.mediaType == "audio" ? "waveform" : "figure.mind.and.body")
                            .font(.system(size: 32))
                            .foregroundColor(Color(red: 0.40, green: 0.30, blue: 0.85))
                    )
                ...
                VStack(alignment: .leading, spacing: 4) {
                    ...
                    Text("\(content.durationMinutes) min • \(content.difficultyLevel.capitalized)")
                        .font(.system(size: 12))
