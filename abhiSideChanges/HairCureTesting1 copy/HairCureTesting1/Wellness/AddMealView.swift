...
struct AddMealView: View {
    ...
    @State private var selectedFoodType: String = "All" // "All", "Vegetarian", "Non-Vegetarian"
    ...
    // Foods available for this meal type (search-filtered & correctly ranked)
    private var suggestedFoods: [Food] {
        guard let e = entry else { return [] }
        let isVegOnly = (selectedFoodType == "Vegetarian")
        let all = store.rankedFoods(for: e.mealType, vegetarianOnly: isVegOnly)
        
        let filteredByType = selectedFoodType == "Non-Vegetarian" ? all.filter { !$0.isVegetarian } : all
        
        if searchText.isEmpty { return filteredByType }
        return filteredByType.filter { $0.name.localizedCaseInsensitiveContains(searchText) }
    }
    ...
    private var searchBar: some View {
        HStack(spacing: 10) {
            ...
            Menu {
                Button("All") { selectedFoodType = "All" }
                Button("Vegetarian") { selectedFoodType = "Vegetarian" }
                Button("Non-Vegetarian") { selectedFoodType = "Non-Vegetarian" }
            } label: {
                Image(systemName: selectedFoodType == "All" ? "line.3.horizontal.decrease.circle" : "line.3.horizontal.decrease.circle.fill")
                    .foregroundColor(selectedFoodType == "All" ? .secondary : mealColor)
            }
        }
