...
struct MindEasePlayerView: View {
    ...
    var body: some View {
                        ...
                        HStack(alignment: .center) {
                            ...
                            Button {
                                withAnimation(.spring(response: 0.3, dampingFraction: 0.6)) {
                                    isFavorited.toggle()
                                    let _ = store.toggleFavorite(contentId: content.id, contentType: "mindEase")
                                }
                            } label: {
        ...
        .onAppear {
            isFavorited = store.isFavorited(contentId: content.id, contentType: "mindEase")
            store.startSession(contentId: content.id)
        }
        .onDisappear {
            let mins = max(1, elapsed / 60)
            store.completeSession(contentId: content.id, minutesCompleted: mins)
        }
    }
}
