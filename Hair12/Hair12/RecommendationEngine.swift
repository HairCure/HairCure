...
struct RecommendationEngine {
    ...
    private static func createTodaysPlan(
        ...
        guard let category = store.mindEaseStore.mindEaseCategories.first(where: { $0.title == categoryTitle }),
    ...
    static func buildDailyProgressSummary(store: AppDataStore) -> DailyProgressSummary {
        let cal        = store.dietMateStore.todaysTotalCalories()
        let calTarget  = store.activeNutritionProfile?.tdee ?? 2000
        let mindful    = Float(store.mindEaseStore.todaysMindfulMinutes())
