...
//
//  Business logic, seed data, computed properties, and query helpers.
//  No SwiftUI imports.
...
final class MindEaseDataStore {
    ...
    private func addContent() {
        ...
        mindEaseCategoryContents = [
            ...
            MindEaseCategoryContent(
                id: UUID(), categoryId: yogaId,
                title: "Mukha Svanasana", caption: "Downward Facing Dog",
            ...
            MindEaseCategoryContent(
                id: UUID(), categoryId: yogaId,
                title: "Balayam", caption: "Nail Rubbing",
            ...
            MindEaseCategoryContent(
                id: UUID(), categoryId: meditationId,
                title: "Body Scan Meditation", caption: "Full Body Relaxation",
            ...
            MindEaseCategoryContent(
                id: UUID(), categoryId: meditationId,
                title: "Guided Visualisation", caption: "Positive Imagery",
    ...
    private func seedTodaysPlan(userId: UUID, userPlans: [UserPlan]) {
        ...
        let entries: [(String, Int)] = [
            ("Bhramari",        plan.meditationMinutesPerDay),
            ("Anulom Vilom",    15),
            ("Uttanasana",      plan.yogaMinutesPerDay),
            ("Mukha Svanasana", 15),
            ("Balayam",         10),
            ("Ocean Waves",     plan.soundMinutesPerDay),
            ("Forest Breeze",   10),
        ]

        todaysPlans = entries.compactMap { contentTitle, target in
            guard let content = mindEaseCategoryContents.first(where: { $0.title == contentTitle })
            else { return nil }
            return TodaysPlan(
