...
////  Called once after the user completes assessment + hair analysis.
////  Produces: UserPlan + UserNutritionProfile + updated ScanReport
////
////  Flow:
////  1. Score lifestyle answers  → 4 dimension scores → composite → LifestyleProfile
////  2. Read stage from scan     → HairFallStage
////  3. Cross-reference matrix   → planId (1A–3C or refer_doctor)
////  4. Apply scalp modifier     → scalpModifier on UserPlan
////  5. Assign MindEase sessions → minutes per category per plan
////  6. Run BMR pipeline         → UserNutritionProfile + per-slot budgets
////  7. Write everything to store
////
//
//import Foundation
//
...
//        store.mealEntries.removeAll(where: {
//            $0.userId == output.userPlan.userId &&
...
//            store.mealEntries.append(MealEntry(
...
//        store: AppDataStore
//    ) {
//        store.todaysPlans.removeAll(where: {
...
//
//        guard let category = store.mindEaseCategories.first(where: { $0.title == categoryTitle }),
//              let content  = store.mindEaseCategoryContents
...
//        }
//
//        store.todaysPlans.append(TodaysPlan(
...
//            planId: plan?.planId ?? "–", daysOnPlan: days
//        )
//    }
//
//    // ─────────────────────────────────────────────
