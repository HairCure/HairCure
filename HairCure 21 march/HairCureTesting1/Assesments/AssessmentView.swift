...
//  HairCure
//
//  Drives the full 12-question assessment flow.
//  Question types handled:
//    .singleChoice  →  option pill list (one tap selects, fills brown)
//    .multiChoice   →  option pill list (multi-select)
//    .picker        →  text field + wheel picker (age / height / weight)
//    .imageChoice   →  2×2 image grid (fallback stage picker)
//    .freeText      →  plain text field
//
//  After Q12 → calls store.completeAssessment() → onComplete()
//
...
struct AssessmentView: View {
    ...
    var body: some View {
        ZStack(alignment: .bottom) {
            ...
            VStack(spacing: 0) {
                ...
                // Animate only the progress bar, not the whole view
                HCProgressBar(current: currentIndex + 1, total: totalCount)
                    .animation(.easeInOut(duration: 0.30), value: currentIndex)
                    .padding(.bottom, 32)

                if let q = currentQuestion {
                    // Give each question a unique identity so SwiftUI
                    // always builds a fresh view — prevents stale state
                    // and fixes the progress bar freeze on fast taps.
                    questionBody(q)
                        .id(currentIndex)
        ...
        // Only animate question body transitions, not the whole view
    ...
    private var headerBar: some View {
        HStack {
            ...
            if currentQuestion?.scoreDimension != .none {
                Button("Skip") { advance() }
                    .font(.system(size: 16))
                ...
                Button {
                    ...
                    .overlay(
                        RoundedRectangle(cornerRadius: 14)
                            .stroke(
                                selected == opt.id ? Color.clear : Color(.systemGray4),
                                lineWidth: 1
                            )
                    )
                }
        ...
        return VStack(spacing: 12) {
            ForEach(opts) { opt in
                ...
                Button {
                    ...
                    store.saveMultiAnswer(questionId: q.id,
                                         selectedOptionIds: Array(current))
                } label: {
                    HStack {
                        Text(opt.optionText)
    ...
    private func stageImage(imageURL: String?, index: Int) -> some View {
        if let url = imageURL, !url.isEmpty, UIImage(named: url) != nil {
            ...
            // Placeholder when asset not yet in catalog
            ZStack {
                Color(.systemGray6)
                VStack(spacing: 6) {
    ...
    private func freeTextQuestion(for q: Question) -> some View {
        ...
        return TextField("Your answer", text: binding)
            .hcInputField()
    }
    ...
    private var canContinue: Bool {
        ...
        switch q.questionType {
        case .singleChoice:
            return singleSelections[q.id] != nil
        case .multiChoice:
            return !(multiSelections[q.id]?.isEmpty ?? true)
        case .picker:
            return pickerValues[q.id] != nil
        case .imageChoice:
            return imageSelections[q.id] != nil
        case .freeText:
    ...
    /// Pre-seed picker values to the midpoint so the wheel renders correctly on first appear
