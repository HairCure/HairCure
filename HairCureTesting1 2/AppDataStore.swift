...
class AppDataStore {
    ...
    /// Owned here so the app entry point only needs to inject one parent store,
    /// then also inject `store.hairInsightsStore` alongside it in the environment.
    ...
    private func seedFoods() {
        ...
        let seeds: [FoodSeed] = [
            FoodSeed(name:"Paneer Stuffed Paratha",      img:"hairInsight",    isVeg:true,  calMin:300,calMax:330,protein:12,carbs:38,fat:12,vitamins:4.5,serving:200, biotin:true, zinc:false,iron:false,omega3:false,vitA:false, meals:[.breakfast,.dinner]),
